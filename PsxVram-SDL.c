...
void drawInbound(SDL_Surface* sur, SDL_Window* win, SDL_Surface* winSur, u16* pInBuffer, SDL_Rect* rect)
{
	...
	if (mode == SDL_PIXELFORMAT_INDEX8)
	{
		for (i = 0; i < h; i++)
		{
			//copy scanline from buffer. Fast!
			memcpy(dst, src, w * 2);
			src += VRAM_WIDTH * 2;
			dst += w * 2;
		}
...
void updateTitle(SDL_Window* win, SDL_Window* win2, SDL_Rect* rect, line* clutLine)
{
	...
	if ((mode == SDL_PIXELFORMAT_INDEX4MSB) || (mode == SDL_PIXELFORMAT_INDEX8))
	{
		...
		{
			strcat(strBuf2, "CLUT ");
			sprintf(clutStrBuf, "CLUT(X:%03d, Y:%03d, Offset:0x%05X)", clutLine->startCoord.x, clutLine->startCoord.y,
			        (clutLine->startCoord.x + clutLine->startCoord.y * VRAM_WIDTH) * 2);
		}
		else
		{
	...
	sprintf(strBuf, "(X:%03d, Y:%03d, Offset:0x%05X) [W:%03d, H:%03d] %s", rect->x, rect->y,
	        (rect->x + rect->y * VRAM_WIDTH) * 2, rect->w, rect->h, clutStrBuf);
...
void updatePal(SDL_Surface* sur, line* clutLine, SDL_Surface* surOut)
{
	...
	length = clutLine->length = (mode == SDL_PIXELFORMAT_INDEX8) ? CLUT_SIZE_8BPP : CLUT_SIZE_4BPP;
	//4bpp has 0x10 colors in pal, 8bpp - 0x100

	if (clutFlag == 0)
	{
		//fill greyscale values
...
void showMouseCoords(int x, int y, SDL_Rect* rect, SDL_Window* win)
{
	...
	switch (mode)
	{
	//calculate global mouse coords based on relative coords and rect position
	case SDL_PIXELFORMAT_INDEX4MSB:
...
int main(int argc, char* argv[])
{
	...
	window = SDL_CreateWindow("VRAM overview", SDL_WINDOWPOS_UNDEFINED, 32, VRAM_WIDTH, VRAM_HEIGHT,
	...
	window2 = SDL_CreateWindow("Mode viewer", SDL_WINDOWPOS_UNDEFINED, y + VRAM_HEIGHT, VRAM_WIDTH, VRAM_HEIGHT,
	                           SDL_WINDOW_SHOWN | SDL_WINDOW_ALLOW_HIGHDPI);
...
readFile:
	...
	fclose(fIn);
	//overview window       
	sur15 = SDL_CreateRGBSurfaceFrom((u8*)pInBuffer, VRAM_WIDTH, VRAM_HEIGHT, 16, VRAM_WIDTH * sizeof(u16), 0x1F, 0x3E0,
	                                 0x7c00, 0x0);
	...
	sur24 = SDL_CreateRGBSurfaceFrom((u8*)pInBuffer24, VRAM_WIDTH_24BPP, VRAM_HEIGHT, 32, VRAM_WIDTH_24BPP * 4, 0xFF,
	                                 0xFF00, 0xFF0000, 0x0);
	...
	while (running)
	{
		...
		case SDL_MOUSEMOTION:
			if ((event.motion.windowID == 2) && (SDL_GetModState() & KMOD_CTRL))
			{
				//show coords at any mouse movement and ctrl is pressed
